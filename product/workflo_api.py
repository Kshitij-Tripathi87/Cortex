...
from product.workflo_backend.infrastructure.security import (
    AuthContext,
    get_current_user,
...
from product.workflo_backend.policy import PolicyViolation
from product.workflo_backend.runs import Run, RunNotFound, get_run_store
...
async def inspect_sandbox(
    ...
    try:
        sb = get_orchestrator().get(sandbox_id)
        require_workspace_access(sb.workspace_id, auth)
...
async def agent_continue(
    ...
    if req.run_id:
        ...
        try:
            ...
            require_workspace_access(run.workspace_id, auth)
