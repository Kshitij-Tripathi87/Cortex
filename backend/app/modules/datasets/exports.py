...
class DatasetExporter:
    ...
    def export(
        ...
        if format == "json":
            ...
            if not FEATHER_AVAILABLE:
                raise RuntimeError("pyarrow/feather not installed. Install with: pip install pyarrow")
    ...
    def _csv_to_dataframe(self, filepath: Path):
        ...
        if "safety_stock" in df.columns:
            df["safety_stock"] = pd.to_numeric(df["safety_stock"], errors="coerce").fillna(0).astype("int64")
        ...
        if "lead_time_days" in df.columns:
            df["lead_time_days"] = pd.to_numeric(df["lead_time_days"], errors="coerce").fillna(0).astype("int64")
        if "throughput_per_day" in df.columns:
            df["throughput_per_day"] = pd.to_numeric(df["throughput_per_day"], errors="coerce").fillna(0).astype("int64")
        if "weight" in df.columns:
            df["weight"] = pd.to_numeric(df["weight"], errors="coerce")
        if "risk_score" in df.columns:
            df["risk_score"] = pd.to_numeric(df["risk_score"], errors="coerce")
        if "contract_value_annual" in df.columns:
            df["contract_value_annual"] = pd.to_numeric(df["contract_value_annual"], errors="coerce")
