...
class DecisionTransitionDB(Base):
    ...
    transition_id: Mapped[str] = mapped_column(String(64), unique=True, default=lambda: f"TRN-{uuid4().hex[:8]}")
    decision_id: Mapped[str] = mapped_column(String(64), ForeignKey("nexus_decisions.decision_id", ondelete="CASCADE"), index=True)
    ...
    decision: Mapped[DecisionRecordDB] = relationship(back_populates="transitions")
...
class ExecutionRecordDB(Base):
    ...
    status: Mapped[str] = mapped_column(String(40), default="pending", index=True)  # pending|executing|executed|failed|rolled_back
...
class OutcomeRecordDB(Base):
    ...
    decision_id: Mapped[str] = mapped_column(String(64), ForeignKey("nexus_decisions.decision_id", ondelete="CASCADE"), index=True)
    execution_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("nexus_executions.execution_id", ondelete="SET NULL"), nullable=True)
    tenant_id: Mapped[str] = mapped_column(String(64), index=True)
    workspace_id: Mapped[str] = mapped_column(String(64), index=True)
...
class ObservationRecordDB(Base):
    ...
    observation_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: f"OBS-{uuid4().hex[:10]}")
    forecast_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("nexus_forecasts.forecast_id", ondelete="SET NULL"), nullable=True, index=True)
    tenant_id: Mapped[str] = mapped_column(String(64), index=True)
    workspace_id: Mapped[str] = mapped_column(String(64), index=True)
    ...
    observation_type: Mapped[str] = mapped_column(String(40), default="demand")  # demand|sla|eta|cost
...
class RiskRecordDB(Base):
    ...
    status: Mapped[str] = mapped_column(String(40), default="open", index=True)  # open|mitigated|closed|stale
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utc_now, index=True)
...
class ScenarioRecordDB(Base):
    ...
    decision_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("nexus_decisions.decision_id", ondelete="SET NULL"), nullable=True)
...
class EvidenceNodeDB(Base):
    ...
    tenant_id: Mapped[str] = mapped_column(String(64), index=True)
    workspace_id: Mapped[str] = mapped_column(String(64), index=True)
    node_type: Mapped[str] = mapped_column(String(40))  # observation|signal|forecast|risk|scenario|decision|approval|execution|outcome|claim
...
class EvidenceEdgeDB(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    from_node_id: Mapped[str] = mapped_column(String(64), ForeignKey("nexus_evidence_nodes.node_id", ondelete="CASCADE"), index=True)
    to_node_id: Mapped[str] = mapped_column(String(64), ForeignKey("nexus_evidence_nodes.node_id", ondelete="CASCADE"), index=True)
    relation: Mapped[str] = mapped_column(String(64))  # supports|causes|informs|contradicts|proves|derived_from
...
class VanessaSessionDB(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utc_now)
    last_active_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utc_now, onupdate=_utc_now)

    messages: Mapped[list["VanessaMessageDB"]] = relationship(
        back_populates="session", cascade="all, delete-orphan", order_by="VanessaMessageDB.turn_number"
    )
...
class VanessaMessageDB(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    message_id: Mapped[str] = mapped_column(String(64), unique=True, default=lambda: f"VMSG-{uuid4().hex[:10]}")
    session_id: Mapped[str] = mapped_column(String(64), ForeignKey("nexus_vanessa_sessions.session_id", ondelete="CASCADE"), index=True)
    turn_number: Mapped[int] = mapped_column(Integer)
    role: Mapped[str] = mapped_column(String(20))  # user|assistant|system|tool
    content: Mapped[str] = mapped_column(Text)
    response_blocks: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON, nullable=True)  # TEXT|METRIC|TABLE|GRAPH|TIME_SERIES|RISK_CARD|SCENARIO_MATRIX|DECISION_CARD|EVIDENCE_CHAIN|TIMELINE
...
class ModelRegistryEntryDB(Base):
    ...
    model_type: Mapped[str] = mapped_column(String(64), nullable=False)  # forecast|gnn|rl|risk|eta|sla
    description: Mapped[str] = mapped_column(Text, default="")
    ...
    # Metrics
    metrics: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)  # MAE, RMSE, WAPE, MAPE, etc.
    calibration: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)  # P50/P80/P95 coverage
    gnn_config: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # GNN-specific config
    rl_config: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # RL-specific config

    # Lifecycle
    status: Mapped[str] = mapped_column(String(40), default="training", index=True)  # training|evaluating|shadow|calibrating|approved|deployed|monitoring|rolled_back|archived
    approval_status: Mapped[str] = mapped_column(String(40), default="pending", index=True)  # pending|approved|rejected
...
class EventRecordDB(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    event_id: Mapped[str] = mapped_column(String(64), unique=True, default=lambda: f"EVT-{uuid4().hex[:10]}")
    tenant_id: Mapped[str] = mapped_column(String(64), index=True)
    workspace_id: Mapped[str] = mapped_column(String(64), index=True)
