...
def populated_world():
    ...
    for _ in range(30):
        eng.record_history(
            [HistoricalDemandPoint(timestamp=datetime.now(UTC), sku="SKU-142", quantity=100.0)]
        )

    # Existing decision to memory
    get_decision_memory().record(
        DecisionRecord(
            decision_id="DEC-PREV-1",
            tenant_id=str(TENANT),
            workspace_id=str(WORKSPACE),
            situation="Previous supplier disruption",
            evidence_ids=[],
...
async def test_investigation_demand_forecast_chain(investigator, populated_world):
    ...
    assert any(
        e.get("tool") == "get_forecast" for e in result.evidence_trail if isinstance(e, dict)
    )
