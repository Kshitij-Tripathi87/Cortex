...
def test_closed_vocabulary_covers_supply_chain_kinds():
    expected = {
        "organization", "workspace", "supplier", "manufacturer", "plant",
        "warehouse", "port", "product", "sku", "component", "material",
        "purchase_order", "sales_order", "shipment", "route", "carrier",
        "inventory_position", "customer", "contract", "capacity", "demand",
        "forecast", "lead_time", "disruption", "signal", "scenario",
        "simulation", "decision", "recommendation", "execution", "outcome",
        "evidence", "agent", "tool", "policy", "approval",
    }
...
def test_specialized_entity_factory_creates_correct_kind():
    s = SupplierEntity.create(
        ...
        natural_key="S1", name="S", source="t",
    )
    assert s.kind == EntityKind.SUPPLIER
    w = WarehouseEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="W1", name="W", source="t",
    )
    assert w.kind == EntityKind.WAREHOUSE
    p = PurchaseOrderEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="PO1", name="PO1", source="t",
        quantity=100, supplier_id=uuid4(),
    ...
    fo = ForecastEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="F1", name="F1", source="t",
        horizon_days=14,
        forecast_timestamp=__import__("datetime").datetime.now(__import__("datetime").UTC),
        p50=100, p80=120, p95=140, confidence=0.8,
        primary_drivers=["seasonality"], model_version="Demand-v0.1",
    )
...
def test_repository_upsert_and_get(repo):
    supplier = SupplierEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="S1", name="S1", source="t", capacity_pct=80.0,
    )
...
def test_repository_get_by_natural_key(repo):
    s = SupplierEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="S42", name="S42", source="t",
    )
    repo.upsert(s)
...
def test_repository_query_by_kind_and_text(repo):
    for i in range(3):
        s = SupplierEntity.create(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            natural_key=f"S{i}", name=f"Supplier {i}", source="t",
        )
        repo.upsert(s)
    for i in range(2):
        w = WarehouseEntity.create(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            natural_key=f"W{i}", name=f"Warehouse {i}", source="t",
        )
        repo.upsert(w)
    page = repo.query(
        EntityQuery(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            kinds=[EntityKind.SUPPLIER], limit=10, offset=0,
        )
    )
    assert page.total == 3
    page = repo.query(
        EntityQuery(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            kinds=[EntityKind.WAREHOUSE], limit=10, offset=0,
        )
    )
    assert page.total == 2
    page = repo.query(
        EntityQuery(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            text_search="Supplier 0", limit=10, offset=0,
        )
    )
    assert page.total == 1
...
def test_repository_query_by_domain(repo):
    ...
    fo = ForecastEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="F", name="F", source="t",
        horizon_days=7,
        forecast_timestamp=__import__("datetime").datetime.now(__import__("datetime").UTC),
        p50=10, p80=12, p95=14, confidence=0.5,
        primary_drivers=[], model_version="v0",
    )
    repo.upsert(fo)
    ...
    page = repo.query(
        EntityQuery(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            domain=EntityDomain.COMMERCIAL, limit=10, offset=0,
        )
    )
    ...
    page = repo.query(
        EntityQuery(
            tenant_id=TENANT, workspace_id=WORKSPACE,
            domain=EntityDomain.SUPPLY, limit=10, offset=0,
        )
    )
...
def test_repository_query_isolated_by_tenant(repo):
    ...
    s1 = SupplierEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="S1", name="S1", source="t",
    )
    s2 = SupplierEntity.create(
        tenant_id=other_tenant, workspace_id=WORKSPACE,
        natural_key="S2", name="S2", source="t",
    )
    repo.upsert(s1)
    repo.upsert(s2)
    page = repo.query(
        EntityQuery(tenant_id=TENANT, workspace_id=WORKSPACE, limit=10, offset=0)
    )
    assert page.total == 1
...
def test_repository_traverse_supply_chain(repo):
    ...
    product = ProductEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="P", name="P", source="t",
    )
    repo.upsert(product)
    customer_id = uuid4()
    po = PurchaseOrderEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="PO", name="PO", source="t",
        quantity=10, supplier_id=supplier.entity_id,
        expected_delivery=__import__("datetime").datetime.now(__import__("datetime").UTC),
    )
    so = SalesOrderEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="SO", name="SO", source="t",
        quantity=10, customer_id=customer_id,
        promised_delivery=__import__("datetime").datetime.now(__import__("datetime").UTC),
        revenue=50000.0, sla_risk_pct=0.5,
    )
    repo.upsert(po)
    repo.upsert(so)
...
def test_permission_grant_can_be_attached_to_entity(repo):
    ...
    s.grant_permission(
        PermissionGrant(role="analyst", can_read=True, granted_by="admin")
    )
