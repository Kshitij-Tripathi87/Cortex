...
class TestRiskEngine:
    ...
    def test_risk_aggregates_multiple_signals_same_target(self, populated_world):
        wm = get_world_model()
        supplier = next(
            e for e in wm.iter_entities(TENANT, WORKSPACE) if e.kind == EntityKind.SUPPLIER
        )
        sig2 = SignalEntity.create(
            tenant_id=TENANT,
            workspace_id=WORKSPACE,
            natural_key="SIG-2",
            name="Second outage signal",
            source="test",
            affected_entity_id=supplier.entity_id,
            severity_score=0.5,
            signal_type="capacity_loss",
        ...
        assert (
            any(
                sig2.entity_id_str
                if hasattr(sig2, "entity_id_str")
                else str(sig2.entity_id) in r.signal_ids
                for r in risks
            )
            or len(risks) >= 1
        )
...
class TestScenarioStudio:
    ...
    def test_demand_surge_increases_revenue_risk(self, populated_world):
        wm = get_world_model()
        order = next(
            e for e in wm.iter_entities(TENANT, WORKSPACE) if e.kind == EntityKind.SALES_ORDER
        )
        studio = get_scenario_studio()
        scenario = ScenarioDefinition(
            workspace_id=str(WORKSPACE),
