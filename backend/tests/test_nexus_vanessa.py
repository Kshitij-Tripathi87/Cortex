...
def orchestrator() -> VanessaOrchestrator:
    ...
    so = SalesOrderEntity.create(
        tenant_id=TENANT, workspace_id=WORKSPACE,
        natural_key="SO-1", name="Big Order", source="test",
        quantity=100, customer_id=uuid4(),
        promised_delivery=datetime.now(UTC),
        revenue=250000.0, sla_risk_pct=0.8,
    )
    wm.upsert(so)
    # Seed demand history
    ...
    mem.record(
        DecisionRecord(
            ...
            evidence_ids=["ev-1"], world_state_version=10,
            options=[{"id": "A"}, {"id": "B"}],
            recommended_option_id="B", chosen_option_id="B",
            policy_id="pol-1", approval_id="apr-1",
...
class _EchoTool(Tool):
    ...
    def invoke(self, call: ToolCall) -> ToolResult:
        return ToolResult(
            call_id=call.call_id, tool_name=self.name, ok=True,
...
def test_tool_registry_rejects_unknown_tool():
    ...
    result = r.invoke(ToolCall(
        tool_name="nonexistent", arguments={}, requester_role="analyst",
        requester_id="u", tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
...
def test_tool_registry_rejects_insufficient_role():
    ...
    result = r.invoke(ToolCall(
        tool_name="_admin", arguments={"x": 1}, requester_role="viewer",
        requester_id="u", tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok is False
...
def test_tool_registry_rejects_invalid_arguments():
    ...
    r.register(_EchoTool())
    result = r.invoke(ToolCall(
        tool_name="_echo", arguments={"x": "not_an_int"}, requester_role="analyst",
        requester_id="u", tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok is False
...
def test_tool_registry_handles_exceptions_gracefully():
    ...
    r.register(_Boom())
    result = r.invoke(ToolCall(
        tool_name="_boom", arguments={}, requester_role="analyst",
        requester_id="u", tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok is False
...
def test_query_world_state_tool(orchestrator):
    result = QueryWorldStateTool().invoke(ToolCall(
        tool_name="query_world_state", arguments={"limit": 10},
        requester_role="viewer", requester_id="u",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok
...
def test_query_world_state_rejects_unknown_kind(orchestrator):
    result = QueryWorldStateTool().invoke(ToolCall(
        tool_name="query_world_state", arguments={"kinds": ["banana"]},
        requester_role="viewer", requester_id="u",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok is False
...
def test_traverse_graph_tool(orchestrator):
    ...
    result = TraverseGraphTool().invoke(ToolCall(
...
def test_get_signal_tool_returns_empty_when_no_signals(orchestrator):
    result = GetSignalTool().invoke(ToolCall(
        tool_name="get_signal", arguments={"min_severity": 0.5},
        requester_role="viewer", requester_id="u",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok
...
def test_get_forecast_produces_probabilistic_output(orchestrator):
    result = GetForecastTool().invoke(ToolCall(
        tool_name="get_forecast", arguments={"sku": "SKU-1", "horizon_days": 7},
        requester_role="viewer", requester_id="u",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok
...
def test_get_supplier_risk_returns_suppliers(orchestrator):
    result = GetSupplierRiskTool().invoke(ToolCall(
        tool_name="get_supplier_risk", arguments={"min_risk": 0.0},
        requester_role="viewer", requester_id="u",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok
...
def test_get_orders_at_risk_returns_revenue_exposed(orchestrator):
    result = GetOrdersAtRiskTool().invoke(ToolCall(
        tool_name="get_orders_at_risk", arguments={"min_revenue": 0.0},
        requester_role="viewer", requester_id="u",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    ))
    assert result.ok
...
def test_orchestrator_ask_returns_grounded_answer(orchestrator):
    q = VanessaQuery(
        ...
        requester_id="alice", requester_role="analyst",
        tenant_id=TENANT, workspace_id=WORKSPACE,
    )
...
def test_orchestrator_unknown_intent_returns_help(orchestrator):
    ...
    assert "could not" in answer.rendered_answer.lower() or "try asking" in answer.rendered_answer.lower()
...
def test_default_registry_has_all_expected_tools():
    ...
    assert {
        "query_world_state", "traverse_graph", "get_signal",
        "get_blast_radius", "get_forecast", "compare_forecast_actual",
        "get_supplier_risk", "get_orders_at_risk", "get_decision",
